...
list(
  color = "#EBE6E0",
  value = scales::label_number()(sum(stars$stars))
)
```
...
pkg_versions |> 
  ...
  ggplot(aes(x = date, y = count, color = `Dependency type`)) +
    ...
    scale_color_manual(values = c(Soft = "#10BED2", Hard = "#AEC800"))
```
...
full_join(maintainers_by_year, authors_by_year, by = "name") |>
  ...
  ggplot(aes(x = year, fill = role)) +
    ...
    labs(
      title = "Recruitement of new authors & maintainers",
      x = "Year",
