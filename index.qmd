...
# Authorship trends and sustainability
...
pkg_versions |> 
  ...
  summarise(
    maintainer_change = n_distinct(stringr::str_extract(Maintainer, "^[^ ]+")) - 1
  ) |> 
  ggplot(aes(x = maintainer_change)) +
    geom_histogram() +
    labs(x = "Number of maintainer transitions over package lifetime")
```

```{r}
maintainers <- pkg_versions |> 
  group_by(name = trimws(stringr::str_extract(Maintainer, "^[^<]+"))) |> 
  summarise(
...
authors <- pkg_versions |> 
  ...
  dplyr::mutate(auts = stringr::str_trim(auts)) |>
  group_by(name = auts) |> 
  summarise(
...
```{r}
pkg_versions |> 
  dplyr::mutate(first_release = Version == min(Version), .by = Package) |>
